...
class DataMonitor:
    ...
    async def _check_data_quality(self) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Data quality check completed for {len(property_list)} properties")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        self.logger.debug("Alert webhook notification sent successfully")
                    else:
                        self.logger.warning(
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": "healthy" if error_rate < 0.1 and avg_response_time < 15 else "degraded"
        }
