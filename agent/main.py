...
class WFRMLSAgent:
    ...
    async def _sync_properties(self, cutoff_time: datetime) -> None:
        ...
        try:
            ...
            properties = self.client.property.get_properties(
                filter_query=filter_query, top=200
            )
    ...
    async def _sync_members(self, cutoff_time: datetime) -> None:
        ...
        try:
            filter_query = f"ModificationTimestamp gt {cutoff_time.isoformat()}Z"
            members = self.client.member.get_members(filter_query=filter_query, top=200)
    ...
    async def _sync_offices(self, cutoff_time: datetime) -> None:
        ...
        try:
            filter_query = f"ModificationTimestamp gt {cutoff_time.isoformat()}Z"
            offices = self.client.office.get_offices(filter_query=filter_query, top=200)
    ...
    async def _process_deletions(self) -> None:
        ...
        try:
            ...
            deleted_records = self.client.deleted.get_deleted(top=200)

            processed_count = await self.data_processor.process_deletions(
                deleted_records
            )
