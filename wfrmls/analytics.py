...
class WFRMLSAnalytics:
    ...
    def get_market_summary(
        ...
        try:
            ...
            new_listing_filter = f"{base_filter} and ListingContractDate gt {cutoff_str}"
            new_listings = self.client.property.get_properties(
                filter_query=new_listing_filter,
                top=200
            )
            ...
            # Price analysis
            prices = [prop.get('ListPrice', 0) for prop in active_properties if prop.get('ListPrice')]
            ...
            for prop in active_properties:
                ...
                if list_date:
                    try:
                        ...
                        days_on_market = (datetime.utcnow().replace(tzinfo=list_dt.tzinfo) - list_dt).days
            ...
            return {
                ...
                "inventory": {
                    ...
                    "new_listings_rate": len(new_properties) / days_back if days_back > 0 else 0
                },
                ...
                "activity": {
                    ...
                    "market_velocity": 1 / (avg_dom / 30) if avg_dom > 0 else 0  # Turnover per month
                }
            }
    ...
    def analyze_price_trends(
        ...
        try:
            ...
            properties = self.client.property.get_properties(
                filter_query=base_filter,
                top=200,
                select=["ListPrice", "BedroomsTotal", "BathroomsTotalInteger", "LivingArea", "ListingContractDate"]
            )

            property_list = properties.get('value', [])
            
            # Extract prices and filter out invalid values
            valid_properties = [prop for prop in property_list if prop.get('ListPrice', 0) > 0]
            ...
            for i in range(price_segments):
                ...
                end_idx = (i + 1) * segment_size if i < price_segments - 1 else len(prices)
                
                segment_prices = prices[start_idx:end_idx]
                segment_name = f"Segment {i + 1}" if price_segments > 3 else ["Budget", "Mid-Range", "Luxury"][i] if i < 3 else f"Premium {i - 2}"
            ...
            return {
                ...
                "market_insights": {
                    "luxury_threshold": prices[int(len(prices) * 0.8)] if len(prices) > 5 else max(prices),
                    "affordable_threshold": prices[int(len(prices) * 0.2)] if len(prices) > 5 else min(prices),
                    "price_diversity_score": (max(prices) - min(prices)) / avg_price if avg_price > 0 else 0
                }
            }
    ...
    def generate_agent_performance_report(
        ...
        try:
            ...
            recent_listings = self.client.property.get_properties(
                ...
                expand="Member",
                top=200,
                select=["ListingId", "ListPrice", "StandardStatus", "ListingContractDate"]
            )
            ...
            for listing in listings:
                ...
                member_key = member_info.get('MemberKey') if isinstance(member_info, dict) else None
            ...
            for agent_name, stats in agent_stats.items():
                if stats['listing_count'] >= min_listings:
                    stats['avg_price'] = stats['total_value'] / stats['listing_count'] if stats['listing_count'] > 0 else 0
                    stats['name'] = agent_name
                    qualified_agents.append(stats)

            # Sort agents by different metrics
            top_by_listings = sorted(qualified_agents, key=lambda x: x['listing_count'], reverse=True)
            top_by_value = sorted(qualified_agents, key=lambda x: x['total_value'], reverse=True)
            top_by_avg_price = sorted(qualified_agents, key=lambda x: x['avg_price'], reverse=True)
            ...
            return {
                ...
                "summary": {
                    "avg_listings_per_agent": mean([agent['listing_count'] for agent in qualified_agents]) if qualified_agents else 0,
                    "avg_listing_value": mean([agent['avg_price'] for agent in qualified_agents]) if qualified_agents else 0,
                    "total_market_value": sum([agent['total_value'] for agent in qualified_agents]),
                },
    ...
    def get_data_quality_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            properties = self.client.property.get_properties(
                top=50,
                select=["ListingId", "ListPrice", "StandardStatus", "City", "BedroomsTotal", "LivingArea", "ListingContractDate"]
            )
            ...
            price_missing = sum(1 for prop in property_list if not prop.get('ListPrice'))
            city_missing = sum(1 for prop in property_list if not prop.get('City'))
            bedrooms_missing = sum(1 for prop in property_list if not prop.get('BedroomsTotal'))
            sqft_missing = sum(1 for prop in property_list if not prop.get('LivingArea'))
            
            property_completeness = 1 - (price_missing + city_missing + bedrooms_missing + sqft_missing) / (total_properties * 4)
            
            if price_missing > total_properties * 0.1:
                issues.append(f"High missing price data: {price_missing}/{total_properties} properties")
                recommendations.append("Review price data entry processes")
                
            if city_missing > total_properties * 0.05:
                issues.append(f"Missing city data: {city_missing}/{total_properties} properties")
            ...
            email_missing = sum(1 for member in member_list if not member.get('MemberEmail'))
            
            member_completeness = 1 - email_missing / total_members if total_members > 0 else 1
            
            if email_missing > total_members * 0.2:
                issues.append(f"High missing email data: {email_missing}/{total_members} members")
            ...
            return {
                ...
                "status": "GOOD" if overall_score > 0.9 else "FAIR" if overall_score > 0.7 else "NEEDS_ATTENTION"
            }
